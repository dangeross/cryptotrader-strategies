...
class Market
    ...
    update: (portfolios, instruments, options) ->
        ...
        for pair in @pairs
            ...
            if @ticks % 240 == 0
                pair.report(portfolio, options)
        debug "***** DEMA: #{_.map(@pairs, (pair) ->
            instrument = datasources.get(pair.market, pair.name, pair.interval)
            "#{instrument.asset()} #{Helpers.toFixed(pair.dema)}"
        ).join(', ')}"
...
class Pair
    ...
    update: (portfolio, market, options) ->
        ...
        dema7 = Indicators.dema(instrument.close, 7)
        dema14 = Indicators.dema(instrument.close, 14)
        dema21 = Indicators.dema(instrument.close, 21)
        dema35 = Indicators.dema(instrument.close, 35)
        dema49 = Indicators.dema(instrument.close, 49)

        dema = Helpers.last(dema14) - Helpers.last(dema35) 
        demaLast = Helpers.last(dema14, 1) - Helpers.last(dema35, 1) 
        
        plot
            dema7: _.last(dema7)
            dema14: _.last(dema14)
            dema21: _.last(dema21)
            dema35: _.last(dema35)
            dema49: _.last(dema49)

        if not @dema or dema != @dema
            @dema = dema

            if (pairOptions.trade == 'Both' or pairOptions.trade == 'Buy') and demaLast <= 0 and @dema > 0
                # Buy
                for count in [1..options.iceTrades]
                    @buy(portfolio, market, instrument, options)
            else if (pairOptions.trade == 'Both' or pairOptions.trade == 'Sell') and demaLast >= 0 and @dema < 0
                # Sell
                ticker = trading.getTicker instrument
